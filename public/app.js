...
async function initApp() {
  ...
  setupEventListeners();
  setupAuthListeners();
  void registerCampusPulseServiceWorker();
...
function setupWebSocket() {
  ...
  // Keep the handshake on the shared API proxy path in the Vite artifact.
    ...
    try {
      ...
      if (msg.type === "NEW_ANNOUNCEMENT") {
        ...
        if (matchesPreferredTopic(notice) && isRelevantToCurrentUser(notice)) {
          ...
          // Web Push is delivered by the service worker when enabled. Keep the
          // direct notification as a fallback for browsers without Push API.
          if (!pushNotificationsReady) sendBrowserNotification(notice);
        }
...
async function registerCampusPulseServiceWorker() {
  ...
  try {
    ...
    console.warn("Background notification worker unavailable:", err);
    return null;
  }
}

async function enablePushNotifications({ requestPermission = false } = {}) {
  if (!currentUser || !("Notification" in window) || !("PushManager" in window)) {
    return false;
  }
  ...
  try {
    const registration =
      serviceWorkerRegistration || await registerCampusPulseServiceWorker();
    ...
    if (!saveResponse.ok) throw new Error("Push subscription could not be saved");

    pushNotificationsReady = true;
    return true;
  } catch (err) {
    pushNotificationsReady = false;
    console.warn("Background push notifications are unavailable:", err);
    return false;
  }
}

// Fire a direct OS-level notification as a fallback for browsers without Push API.
...
function maybeRequestNotificationPermission() {
  ...
  Notification.requestPermission().then(permission => {
    if (permission === "granted") void enablePushNotifications();
  });
}
  ...
  document.getElementById("open-notif-prefs-btn")?.addEventListener("click", () => {
    ...
    if (statusEl && "Notification" in window) {
      if (pushNotificationsReady) {
        statusEl.textContent = "Background push notifications are enabled, including when you are using another app.";
      } else if (Notification.permission === "granted") {
        statusEl.textContent = "Desktop permission is enabled. Connecting background push notifications...";
        void enablePushNotifications().then(enabled => {
          if (enabled) {
            statusEl.textContent = "Background push notifications are enabled, including when you are using another app.";
          } else {
            statusEl.textContent = "This browser allowed notifications but does not support background push. Keep CampusPulse open for live alerts.";
          }
        });
      } else if (Notification.permission === "denied") {
        statusEl.textContent = "Notifications are blocked in your browser settings. Allow them for background alerts.";
      } else {
        statusEl.innerHTML = `<a href="#" id="enable-browser-notif-link" style="color: var(--primary); font-weight:600;">Enable background notifications</a> to receive alerts while using another app.`;
        document.getElementById("enable-browser-notif-link")?.addEventListener("click", (e) => {
          e.preventDefault();
          void enablePushNotifications({ requestPermission: true }).then(enabled => {
            statusEl.textContent = enabled
              ? "Background push notifications are enabled, including when you are using another app."
              : "Notifications were not enabled. Check your browser permission settings and try again.";
          });
        });
      }
    } else if (statusEl) {
      statusEl.textContent = "This browser does not support background notifications. In-app alerts will still work.";
    }
